...
const ConfirmNoteDeletion = ({onCloseModal, onConfirmDelete, deletedNote}) => {
  ...
  const message = deletedNote.length > 1 ? 'Vous etês certain de supprimer ces notes?' : 'Vous etês certain de supprimer cette note?'
  ...
  return (
    <div className='bg-gray-100 rounded px-2 py-1 md:px-4 md:py-3 dark:border-none dark:bg-header-card-dark'>
      ...
      <footer className='px-2 mt-2 flex gap-4 justify-end'>
        ...
        <button onClick={() => handleConfirm(deletedNote)} className='button-base bg-red-600 hover:bg-red-700 text-gray-100 dark:text-gray-200'>Supprimer</button>  
      </footer>
    </div>
  )
}
